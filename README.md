...
1. Из файла `.env.example` создайте файл `.env.docker` и укажите следующие переменные
   - `AWS_ACCESS_KEY_ID` - Ключ доступа от сервисного аккаунта
   - `AWS_SECRET_ACCESS_KEY` - Секретный код от сервисного аккаунта
   - `AWS_DEFAULT_REGION` - Название региона (уточните у провайдера)
   - `AWS_BUCKET_NAME` - Название вашего бакета (необходимо создать самостоятельно)
2. Также вы можете изменить дефолтное значение переменной `JWT_LIFETIME` - время жизни токена авторизации (в секундах)
3. Выполнить команду `make runserver`
4. Проверить работу API вы сможете тут `http://127.0.0.1/api/openapi`

### Для тестирования в докере

Выполните команду `make test` для запуска тестов сервиса. 

Для работы тестов необходимо чтобы ранее хоть раз уже создавалась БД. 
Для этого вы можете либо запустить проект полностью `docker-compose up`, либо `docker-compose up filipp_sprint_5_db`.
Для дальнейшего тестирования данное действие более не потребуется.

Выполните команду `make pep8` для запуска линтера кода

### Для разработки

1. Из файла `.env.example` создайте файл `.env` и укажите необходимые переменные (смотри инструкцию выше)
   - В качестве адреса БД укажите `localhost` 
2. Создайте виртуальное окружение и установите все зависимости
3. Выполните команду `make runlocal` для старта проекта

### Работа с миграциями

1. Для применения миграций выполните `make migrate`
2. Для отката последней миграции выполните `make rollback_migrations`
3. Сами файлы миграций создаются командой `alembic -c src/alembic.ini revision --autogenerate -m migration_name`
