...
### Для запуска сервера в докере

1. Из файла `.env.example` создать файл `.env.docker`
2. Заменить значение переменных `AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY, AWS_DEFAULT_REGION, AWS_BUCKET_NAME`
3. Выполнить команду `docker-compose up -d`
4. Проверить работу API вы сможете тут `http://127.0.0.1/api/openapi`

### Для тестирования в докере

Выполните команду `docker-compose -f docker-compose-test.yml up --exit-code-from filipp_sprint_5_db_test`
